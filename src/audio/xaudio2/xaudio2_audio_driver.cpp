 ...
 ******************************************************************************
 * Notice: Modified for rex compat by worleydl 2026
 */
...
#include "rex/audio/xaudio2/xaudio2_api.h"
...
class XAudio2AudioDriver::VoiceCallback : public api::IXAudio2VoiceCallback {
 public:
  explicit VoiceCallback(rex::thread::Semaphore* semaphore)
...
bool XAudio2AudioDriver::Initialize() {
  ...
  if (xaudio2_module_) {
    ...
    if (xaudio2_create_) {
      ...
      REXLOG_ERROR("XAudio2Create not found in XAudio2_8.dll");
  ...
  if (!xaudio2_module_) {
    ...
    if (xaudio2_module_) {
      ...
      REXLOG_ERROR("Failed to load XAudio 2.8 or 2.7 library DLL");
      return false;
    }
  }
...
bool XAudio2AudioDriver::InitializeObjects(Objects& objects) {
  ...
  if (FAILED(hr)) {
    REXLOG_ERROR("IXAudio2::CreateMasteringVoice failed with 0x{:08X}", hr);
  ...
  if (FAILED(hr)) {
    REXLOG_ERROR("IXAudio2::CreateSourceVoice failed with 0x{:08X}", hr);
    ShutdownObjects(objects);
    return false;
  }
  ...
  if (FAILED(hr)) {
    REXLOG_ERROR("IXAudio2SourceVoice::Start failed with 0x{:08X}", hr);
    ShutdownObjects(objects);
    return false;
  }
...
void XAudio2AudioDriver::SubmitFrame(uint32_t frame_ptr) {
  ...
  if (FAILED(hr)) {
    REXLOG_ERROR("SubmitSourceBuffer failed with 0x{:08X}", hr);
    return;
  }
  ...
  float frequency_ratio = static_cast<float>(chrono::Clock::guest_time_scalar());
  if (api_minor_version_ >= 8) {
...
void XAudio2AudioDriver::MTAThread() {
  //todo: cleanup, whered this get refactored to?
  //rex::thread::set_name("XAudio2 MTA");
  ...
  if (!com_initialized) {
    REXLOG_ERROR("XAudio2 MTA thread CoInitializeEx failed with 0x{:08X}", hr);
  } else {
    if (api_minor_version_ >= 8) {
      hr = xaudio2_create_(&objects_.api_2_8.audio, 0, kProcessor);
      if (FAILED(hr)) {
        REXLOG_ERROR("XAudio2Create failed with 0x{:08X}", hr);
      } else {
      ...
      if (FAILED(hr)) {
        REXLOG_ERROR("CoCreateInstance for XAudio2 failed with 0x{:08X}", hr);
      } else {
        hr = objects_.api_2_7.audio->Initialize(0, kProcessor);
        if (FAILED(hr)) {
          REXLOG_ERROR("IXAudio2::Initialize failed with 0x{:08X}", hr);
        } else {
